...
- **Total Paid** — shown only when `invoice.status === 'paid'`. Equal to Invoice Total (derived — no partial payments concept exists; see below). Shown as label + amount only, no date.
